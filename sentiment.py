...
# Common positive and negative words for better filtering
POSITIVE_WORDS = {
...
def plot_sentiment_pie(sentiment_percentages):
    ...
    fig.update_layout(
        ...
        margin=dict(t=50, l=50, r=50, b=50),  # Add margins
        showlegend=True,
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
def generate_sentiment_wordclouds(df):
    # Configure different word clouds for positive and negative sentiments
    pos_wc = WordCloud(
    ...
    # Create word frequency dictionaries for positive and negative words
    positive_words = defaultdict(int)
    negative_words = defaultdict(int)
    
    # Process each message and its sentiment
    for message, sentiment in zip(df['message'], df['sentiment_score']):
        ...
        for word in words:
            if len(word) > 3:  # Filter out very short words
                ...
                if sentiment > 0.05:
                    # Only include words that are either in our positive word list
                    # or have a positive sentiment on their own
                    word_sentiment = TextBlob(word).sentiment.polarity
                    if word in POSITIVE_WORDS or word_sentiment > 0.2:
                        positive_words[word] += 1
                # For negative sentiment
                elif sentiment < -0.05:
                    # Only include words that are either in our negative word list
                    # or have a negative sentiment on their own
                    word_sentiment = TextBlob(word).sentiment.polarity
                    if word in NEGATIVE_WORDS or word_sentiment < -0.2:
                        negative_words[word] += 1
    
    # Generate wordclouds using the filtered words
